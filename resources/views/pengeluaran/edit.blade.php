...
<div class="container mx-auto p-4 max-w-lg">
    <h1 class="text-2xl font-bold mb-6">Edit Pengeluaran Stok</h1>
    ...
    <form action="{{ route('pengeluaran.update', $pengeluaran->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="item_id" id="item_id" class="w-full border border-gray-300 rounded px-3 py-2 @error('item_id') border-red-500 @enderror">
                ...
                <option value="{{ $item->id }}" {{ $pengeluaran->item_id == $item->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="number" name="jumlah_keluar" id="jumlah_keluar" min="1" value="{{ old('jumlah_keluar', $pengeluaran->jumlah_keluar) }}" class="w-full border border-gray-300 rounded px-3 py-2 @error('jumlah_keluar') border-red-500 @enderror" />
            @error('jumlah_keluar')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="date" name="tanggal_keluar" id="tanggal_keluar" value="{{ old('tanggal_keluar', $pengeluaran->tanggal_keluar->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded px-3 py-2 @error('tanggal_keluar') border-red-500 @enderror" />
